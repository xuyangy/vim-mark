...
function! mark#SearchCurrentMark( isBackward )
	...
	if empty(l:markText)
		if s:lastSearch == -1
			...
			call s:Search(s:pattern[s:lastSearch], a:isBackward, [], 'same-mark')
		endif
	else
		call s:Search(l:markText, a:isBackward, l:markPosition, (l:markIndex ==# s:lastSearch ? 'same-mark' : 'new-mark'))
